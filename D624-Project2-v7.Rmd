...
#dmp: this version used corrplot() from library(pls), which is just a bullseye of the first 2 fields.  Think we want...
#corrplot(correlation_matrix, tl.col = "black", tl.cex = 0.6, order = 'AOE') # color (tl.col) and size (tl.cex) help style the plot.

#dmp: ...forcing the use of corrplot() from library(corrplot) for the more traditional plot
corrplot::corrplot(correlation_matrix, tl.col = "black", tl.cex = 0.4, order = 'AOE') # color (tl.col) and size (tl.cex) help style the plot.)
```
...
y_predicted <- predict(ridge_model_best, s = best_lambda, newx = x)
...
#dmp
#use fitted best model to make predictions
y_predicted <- predict(lasso_model_best, s = best_lambda_lasso, newx = x)
...
student_eval <- student_eval |> select(-PH)
...
hyper_grid <- expand.grid(
  degree = 3:6, 
  nprune = seq(40, 65, length.out = 10) %>% floor()
  )
set.seed(175175327)
marsModel <- train(x = x,
                   y = y,
...
```{r}
preds <- stats::predict(mars_model, x_eval)
